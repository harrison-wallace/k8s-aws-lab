...
CONTROL_PLANE_ID=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=$CONTROL_PLANE_INSTANCE" "Name=instance-state-name,Values=running,stopped" --query "Reservations[*].Instances[*].InstanceId" --output text)
WORKER_ID_1=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=$WORKER_INSTANCE_1" "Name=instance-state-name,Values=running,stopped" --query "Reservations[*].Instances[*].InstanceId" --output text)
WORKER_ID_2=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=$WORKER_INSTANCE_2" "Name=instance-state-name,Values=running,stopped" --query "Reservations[*].Instances[*].InstanceId" --output text)


# Perform action based on user input
case $ACTION in
    start)
        aws ec2 start-instances --instance-ids $CONTROL_PLANE_ID $WORKER_ID_1 $WORKER_ID_2 > /dev/null
        ...
        ;;
    stop)
        aws ec2 stop-instances --instance-ids $CONTROL_PLANE_ID $WORKER_ID_1 $WORKER_ID_2 > /dev/null
        ...
        ;;
    restart)
        aws ec2 reboot-instances --instance-ids $CONTROL_PLANE_ID $WORKER_ID_1 $WORKER_ID_2 > /dev/null
